...
describe('Crawler with server', function() {
  ...
  it('should crawl the correct links', function(done) {
  ...
  it('should give an error when a crawled link gives an error status code', done => crawler.startCrawl(err_config, (url, cb) => cb(function(err) {
    if (err) {
      return done();
    } else {
  ...
  return describe("Additional validation", () => it('should call validate function', function(done) {
...
describe("Replacing getLinks", function() {
  ...
  it('should be able to replace getLinks function', function() {
  ...
  return it('should call LinkFinder if no replacement passed in', function() {
    crawler.getLinks(OK_RES);
...
describe('Crawler', function() {
  ...
  return describe('processResponse', function() {
    ...
    it('should crawl links that have not previously been crawled', function() {
      linkFinder.getLinks.onCall(0).returns(['a', 'b', 'c', 'b']);
      linkFinder.getLinks.onCall(1).returns(['d', 'e', 'c', 'b']);

      crawler.processResponse('parent', OK_RES, null, function() {});
      crawler.processResponse('parent', OK_RES, null, function() {});
    ...
    it('should crawl specified percentage of links', function() {
    ...
    it('should crawl links with uri template', function() {
      linkFinder.getLinks.onCall(0).returns(['/{value1}?query={value2}{&value3}', 'b']);

      crawler.processResponse('parent', OK_RES, {value1: 'one', value2: 'two'}, function() {});
    ...
    it('by default should error from a bad response', done => crawler.processResponse('parent', BAD_RES, {}, function(err) {
      err.should.match(/status 503/);
      return done();
    }));

    it('can recover from a bad response', function(done) {
      crawler.setConfig({recover() { return true; }});
      return crawler.processResponse('parent', BAD_RES, {}, function(err) {
        should.not.exist(err);
        return done();
      });
    });

    return it('does not run validators after recovering', function(done) {
      crawler.setConfig({
        recover() { return true; },
